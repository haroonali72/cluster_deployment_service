...
func GenerateKeyPair(keyName, username string, ctx utils.Context) (utils.KeyPairResponse, error) {
	...
	ctx.SendLogs("Successfully generated sshkeys", models.LOGGING_LEVEL_INFO, string(models.Backend_Logging))
	arr, err1 := ioutil.ReadFile(keyName)
	str := string(arr)
	if err1 != nil {
		ctx.SendLogs(err1.Error(), models.LOGGING_LEVEL_ERROR, string(models.Backend_Logging))
