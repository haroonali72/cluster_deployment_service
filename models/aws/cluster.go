...
func CreateCluster(cluster Cluster_Def, ctx utils.Context) error {
	...
	if err == nil { //cluster found
		ctx.SendLogs("Cluster model: Create - Cluster  already exists in the database: "+cluster.Name, models.LOGGING_LEVEL_ERROR, string(models.Backend_Logging))
	...
	if err != nil {
		ctx.SendLogs("Cluster model: Create - Got error inserting cluster to the database: "+err.Error(), models.LOGGING_LEVEL_ERROR, string(models.Backend_Logging))
		return err
	}
	ctx.SendLogs(" AWS Cluster: "+cluster.Name+" of Project Id: "+cluster.ProjectId+" created ", models.LOGGING_LEVEL_INFO, string(models.Audit_Trails))
	return nil
}
...
func GetCluster(projectId string, ctx utils.Context) (cluster Cluster_Def, err error) {
	...
	if err1 != nil {
		ctx.SendLogs("Cluster model: Get - Got error while connecting to the database: "+err.Error(), models.LOGGING_LEVEL_ERROR, string(models.Backend_Logging))
	...
	ctx.SendLogs(" Get AWS Cluster "+cluster.Name+" of Project Id: "+cluster.ProjectId+"", models.LOGGING_LEVEL_INFO, string(models.Audit_Trails))
	return cluster, nil
}
...
func GetAllCluster(ctx utils.Context, input rbac_athentication.List) (clusters []Cluster_Def, err error) {
	...
	session, err1 := db.GetMongoSession()
	if err1 != nil {
		ctx.SendLogs("Cluster model: GetAll - Got error while connecting to the database: "+err1.Error(), models.LOGGING_LEVEL_ERROR, string(models.Backend_Logging))
	...
	ctx.SendLogs(" Get all AWS Cluster ", models.LOGGING_LEVEL_INFO, string(models.Audit_Trails))
...
func UpdateCluster(cluster Cluster_Def, update bool, ctx utils.Context) error {
	...
	if err != nil {
		ctx.SendLogs("Cluster model: Update - Cluster   does not exist in the database: "+cluster.Name+err.Error(), models.LOGGING_LEVEL_ERROR, string(models.Backend_Logging))
		return err
	}
	if oldCluster.Status == "Cluster Created" && update {
		ctx.SendLogs("Cluster is in runnning state ", models.LOGGING_LEVEL_ERROR, string(models.Backend_Logging))
		return errors.New("Cluster is in runnning state")
	}
	err = DeleteCluster(cluster.ProjectId, ctx)
	if err != nil {
		ctx.SendLogs("Cluster model: Update - Got error deleting cluster: "+err.Error(), models.LOGGING_LEVEL_ERROR, string(models.Backend_Logging))
		return err
	}
	...
	ctx.SendLogs(" AWS Cluster "+cluster.Name+" of Project Id: "+cluster.ProjectId+"updated in database ", models.LOGGING_LEVEL_INFO, string(models.Audit_Trails))
	return nil
}
...
func DeleteCluster(projectId string, ctx utils.Context) error {
	...
	if err != nil {
		...
		ctx.SendLogs("Cluster model: Delete - Got error while connecting to the database: "+err.Error(), models.LOGGING_LEVEL_ERROR, string(models.Backend_Logging))
		return err
	}
	defer session.Close()
	mc := db.GetMongoConf()
	...
	ctx.SendLogs(" Aws Cluster of Project Id: "+projectId+"deleted from database ", models.LOGGING_LEVEL_INFO, string(models.Audit_Trails))
	return nil
}
func PrintError(confError error, name, projectId string, ctx utils.Context, companyId string) {
	if confError != nil {
		ctx.SendLogs(confError.Error(), models.LOGGING_LEVEL_ERROR, string(models.Backend_Logging))
...
func DeployCluster(cluster Cluster_Def, credentials vault.AwsCredentials, ctx utils.Context, companyId string, token string) (confError error) {
	...
	publisher.Notify(cluster.ProjectId, "Status Available", ctx)
	ctx.SendLogs(" AWS Cluster "+cluster.Name+" of Project Id: "+cluster.ProjectId+"deployed ", models.LOGGING_LEVEL_INFO, string(models.Audit_Trails))
	return nil
}
func FetchStatus(credentials vault.AwsProfile, projectId string, ctx utils.Context, companyId string, token string) (Cluster_Def, error) {
	...
	if err != nil {
		ctx.SendLogs("Cluster model: Deploy - Got error while connecting to the database: "+err.Error(), models.LOGGING_LEVEL_ERROR, string(models.Backend_Logging))
		return Cluster_Def{}, err
	}
	...
	if e != nil {
		...
		ctx.SendLogs("Cluster model: Status - Failed to get lastest status "+e.Error(), models.LOGGING_LEVEL_ERROR, string(models.Backend_Logging))
		return Cluster_Def{}, e
	}
	ctx.SendLogs(" AWS Cluster "+cluster.Name+" of Project Id: "+projectId+"fetched ", models.LOGGING_LEVEL_INFO, string(models.Audit_Trails))
	/*	err = UpdateCluster(c)
		if err != nil {
...
func TerminateCluster(cluster Cluster_Def, profile vault.AwsProfile, ctx utils.Context, companyId string) error {
	...
	if pub_err != nil {
		ctx.SendLogs(pub_err.Error(), models.LOGGING_LEVEL_ERROR, string(models.Backend_Logging))

		return pub_err
	}

	if cluster.Status != "Cluster Created" && cluster.Status == "Cluster Termination Failed" {
		ctx.SendLogs("Cluster model: Cluster is not in created state ", models.LOGGING_LEVEL_ERROR, string(models.Backend_Logging))
	...
	publisher.Notify(cluster.ProjectId, "Status Available", ctx)
	ctx.SendLogs("Cluster "+cluster.Name+" of Project Id: "+cluster.ProjectId+"terminated by ", models.LOGGING_LEVEL_INFO, string(models.Audit_Trails))
	return nil
}
func updateNodePool(createdPools []CreatedPool, cluster Cluster_Def, ctx utils.Context) Cluster_Def {
	for index, nodepool := range cluster.NodePools {
		...
		for _, createdPool := range createdPools {
			...
			ctx.SendLogs("Pool "+nodepool.Name+" created ", models.LOGGING_LEVEL_INFO, string(models.Audit_Trails))

		}

		ctx.SendLogs("Cluster model: updated nodes in pools", models.LOGGING_LEVEL_INFO, string(models.Backend_Logging))
...
func GetAWSAmi(credentials vault.AwsProfile, amiId string, ctx utils.Context, token string) ([]*ec2.BlockDeviceMapping, error) {
	...
	if e != nil {
		ctx.SendLogs("Cluster model: Status - Failed to get ami details "+e.Error(), models.LOGGING_LEVEL_ERROR, string(models.Backend_Logging))
...
func EnableScaling(credentials vault.AwsProfile, cluster Cluster_Def, ctx utils.Context, token string) error {
	...
	if e != nil {
		ctx.SendLogs("Cluster model: Status - Failed to enable  scaling"+e.Error(), models.LOGGING_LEVEL_ERROR, string(models.Backend_Logging))

		return e
	}

	ctx.SendLogs("Cluster: "+cluster.Name+" scaled", models.LOGGING_LEVEL_INFO, string(models.Audit_Trails))
