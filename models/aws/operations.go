...
func (cloud *AWS) updateInstanceTags(instance_id *string, nodepool_name string, projectId string, ctx utils.Context) error {
	...
	ctx.SendLogs(out.String(), models.LOGGING_LEVEL_INFO, string(models.Backend_Logging))
	return nil
}
...
func (cloud *AWS) fetchStatus(cluster Cluster_Def, ctx utils.Context, companyId string, token string) (Cluster_Def, error) {
	if cloud.Client == nil {
		err := cloud.init()
		if err != nil {
			ctx.SendLogs("Failed to get latest status"+err.Error(), models.LOGGING_LEVEL_ERROR, string(models.Backend_Logging))
...
func (cloud *AWS) CreateInstance(pool *NodePool, network types.AWSNetwork, ctx utils.Context) (*ec2.Reservation, error, string) {
	...
	*/
	...
	ctx.SendLogs("attaching external volume", models.LOGGING_LEVEL_ERROR, string(models.Backend_Logging))
	if pool.IsExternal {
	...
	if !ok {
		...
		ctx.SendLogs("failed in attaching", models.LOGGING_LEVEL_INFO, string(models.Backend_Logging))
	}
...
func (cloud *AWS) TerminatePool(pool *NodePool, projectId string, ctx utils.Context, companyId string) error {
	...
	if err != nil {
		ctx.SendLogs("Cluster model: Status - Failed to terminate node pool "+err.Error(), models.LOGGING_LEVEL_ERROR, string(models.Backend_Logging))
		return err
	}
...
func (cloud *AWS) getKey(pool NodePool, projectId string, ctx utils.Context, companyId string, token string) (keyMaterial string, err error) {
	...
	if pool.KeyInfo.KeyType == models.NEWKey {
		...
		if err != nil && err.Error() != "not found" {
			...
			ctx.SendLogs("AWSOperations: creating key", models.LOGGING_LEVEL_INFO, string(models.Backend_Logging))
		...
		if err != nil {
			...
			ctx.SendLogs(err.Error(), models.LOGGING_LEVEL_INFO, string(models.Backend_Logging))
...
func (cloud *AWS) mountVolume(ids []*ec2.Instance, ami Ami, key Key, projectId string, ctx utils.Context, companyId string) error {
	...
	for _, id := range ids {
		...
		if errCopy != nil {
			ctx.SendLogs(errCopy.Error(), models.LOGGING_LEVEL_ERROR, string(models.Backend_Logging))
			return errCopy
		}
