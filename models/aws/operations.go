...
func (cloud *AWS) updateInstanceTags(instance_id *string, nodepool_name string, projectId string, ctx utils.Context) error {
	...
	logType := []string{"backend-logging"}
	ctx.SendLogs(out.String(), constants.LOGGING_LEVEL_INFO, logType)
	return nil
}
...
func (cloud *AWS) fetchStatus(cluster Cluster_Def, ctx utils.Context, companyId string, token string) (Cluster_Def, error) {
	if cloud.Client == nil {
		...
		if err != nil {
			...
			logType := []string{"backend-logging"}
			ctx.SendLogs("Failed to get latest status"+err.Error(), constants.LOGGING_LEVEL_ERROR, logType)
...
func (cloud *AWS) CreateInstance(pool *NodePool, network types.AWSNetwork, ctx utils.Context) (*ec2.Reservation, error, string) {
	...
	*/
	...
	logType := []string{"backend-logging"}
	ctx.SendLogs("attaching external volume", constants.LOGGING_LEVEL_ERROR, logType)
	if pool.IsExternal {
	...
	if !ok {
		...
		logType := []string{"backend-logging"}
		ctx.SendLogs("failed in attaching", constants.LOGGING_LEVEL_INFO, logType)
	}
...
func (cloud *AWS) TerminatePool(pool *NodePool, projectId string, ctx utils.Context, companyId string) error {
	...
	if err != nil {
		logType := []string{"backend-logging"}
		ctx.SendLogs("Cluster model: Status - Failed to terminate node pool "+err.Error(), constants.LOGGING_LEVEL_ERROR, logType)
		return err
	}
	utils.SendLog(companyId, "Cluster pool terminated successfully: "+pool.Name, constants.LOGGING_LEVEL_INFO, projectId)
	return nil
}
...
func (cloud *AWS) getKey(pool NodePool, projectId string, ctx utils.Context, companyId string, token string) (keyMaterial string, err error) {
	...
	if pool.KeyInfo.KeyType == models.NEWKey {
		...
		if err != nil && err.Error() != "not found" {
			logType := []string{"backend-logging"}
			ctx.SendLogs(err.Error(), constants.LOGGING_LEVEL_ERROR, logType)
			utils.SendLog(companyId, "Error in getting key: "+pool.KeyInfo.KeyName, constants.LOGGING_LEVEL_INFO, projectId)
			utils.SendLog(companyId, err.Error(), constants.LOGGING_LEVEL_INFO, projectId)
			return "", err
			...
			logType := []string{"backend-logging"}
			ctx.SendLogs("AWSOperations: creating key", constants.LOGGING_LEVEL_INFO, logType)
			utils.SendLog(companyId, "Creating Key "+pool.KeyInfo.KeyName, constants.LOGGING_LEVEL_INFO, projectId)
			...
			if err != nil {
				logType := []string{"backend-logging"}
				ctx.SendLogs(err.Error(), constants.LOGGING_LEVEL_ERROR, logType)
				utils.SendLog(companyId, "Error in key creation: "+pool.KeyInfo.KeyName, constants.LOGGING_LEVEL_INFO, projectId)
				utils.SendLog(companyId, err.Error(), constants.LOGGING_LEVEL_INFO, projectId)
				return "", err
			}
			pool.KeyInfo.KeyMaterial = keyMaterial
			...
			if err != nil {
				logType := []string{"backend-logging"}
				ctx.SendLogs(err.Error(), constants.LOGGING_LEVEL_ERROR, logType)
				utils.SendLog(companyId, "Error in key insertion: "+pool.KeyInfo.KeyName, constants.LOGGING_LEVEL_INFO, projectId)
				utils.SendLog(companyId, err.Error(), constants.LOGGING_LEVEL_INFO, projectId)
				return "", err
			}
		}
		...
		if err != nil {
			logType := []string{"backend-logging"}
			ctx.SendLogs(err.Error(), constants.LOGGING_LEVEL_INFO, logType)
			utils.SendLog(companyId, "Error in getting key: "+pool.KeyInfo.KeyName, constants.LOGGING_LEVEL_ERROR, projectId)
			utils.SendLog(companyId, err.Error(), constants.LOGGING_LEVEL_INFO, projectId)
			return "", err
		}
		...
		if err != nil {
			logType := []string{"backend-logging"}
			ctx.SendLogs(err.Error(), constants.LOGGING_LEVEL_ERROR, logType)
			utils.SendLog(companyId, "Error in importing key: "+pool.KeyInfo.KeyName, constants.LOGGING_LEVEL_INFO, projectId)
			utils.SendLog(companyId, err.Error(), constants.LOGGING_LEVEL_INFO, projectId)
			return "", err
		}
...
func (cloud *AWS) mountVolume(ids []*ec2.Instance, ami Ami, key Key, projectId string, ctx utils.Context, companyId string) error {
	...
	for _, id := range ids {
		...
		if errCopy != nil {
			logType := []string{"backend-logging"}
			ctx.SendLogs(errCopy.Error(), constants.LOGGING_LEVEL_ERROR, logType)
			return errCopy
		}
