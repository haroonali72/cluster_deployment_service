...
func (cloud *GCP) fetchPoolStatus(pool *NodePool) error {
	...
	ctx := context.Background()
	if pool.PoolRole == "master" {
		result, err := cloud.Client.Instances.Get(cloud.ProjectId, cloud.Region+"-"+cloud.Zone, pool.Name).Context(ctx).Do()
		if err != nil {
			beego.Error(err.Error())
			return err
		}
	...
	result, err := cloud.Client.InstanceGroupManagers.ListManagedInstances(cloud.ProjectId, cloud.Region+"-"+cloud.Zone, pool.Name).Context(ctx).Do()
	if err != nil {
		beego.Error(err.Error())
		return err
	}
