...
func CreateTemplate(template Template, ctx utils.Context) (error, string) {
	...
	if err == nil { //template found
		...
		ctx.SendLogs("gcpTemplateModel :"+text+err.Error(), models.LOGGING_LEVEL_ERROR, string(models.Backend_Logging))
...
func GetTemplate(templateName string, ctx utils.Context) (template Template, err error) {
	...
	if err1 != nil {
		ctx.SendLogs("GcpTemplateModel :"+err1.Error(), models.LOGGING_LEVEL_ERROR, string(models.Backend_Logging))
	...
	if err != nil {
		ctx.SendLogs("GcpTemplateModel :"+err.Error(), models.LOGGING_LEVEL_ERROR, string(models.Backend_Logging))
...
func GetAllTemplate(ctx utils.Context) (templates []Template, err error) {
	session, err1 := db.GetMongoSession()
	if err1 != nil {
		ctx.SendLogs("GcpTemplateModel : error connecting to database "+err1.Error(), models.LOGGING_LEVEL_ERROR, string(models.Backend_Logging))
		beego.Error("Template model: GetAll - Got error while connecting to the database: ", err1)
		return nil, err1
	}
...
func DeleteTemplate(templateName string, ctx utils.Context) error {
	...
	if err != nil {
		ctx.SendLogs("GcpTemplateModel : erro with connecting database "+err.Error(), models.LOGGING_LEVEL_ERROR, string(models.Backend_Logging))
