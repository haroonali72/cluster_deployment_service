...
func (c *DOKSClusterController) GetAll() {
	...
	err, data := rbacAuthentication.GetAllAuthenticate("cluster", userInfo.CompanyId, token, models.DOKS, *ctx)
	if err != nil {
		beego.Error(err.Error())
		c.Ctx.Output.SetStatus(400)
		c.Data["json"] = map[string]string{"error": err.Error()}
...
func (c *DOKSClusterController) Delete() {
	...
	if strings.ToLower(cluster.Status.State) == string(models.ClusterCreated) && !forceDelete {
	...
	if cluster.Status.State == string(models.Deploying) && !forceDelete {
	...
	if cluster.Status.State == string(models.Terminating) && !forceDelete {
...
func (c *DOKSClusterController) StartCluster() {
	...
	if cluster.Status.State == "Cluster Created" {
	...
	cluster.Status.State = string(models.Deploying)
	err = doks.UpdateKubernetesCluster(cluster, *ctx)
	if err != nil {
		c.Ctx.Output.SetStatus(500)
		c.Data["json"] = map[string]string{"error": err.Error()}
...
// @Title Start
// @Description Apply cloudplex Agent file to a DOKS cluster
...
func (c *DOKSClusterController) ApplyAgent() {
	...
	ctx := new(utils.Context)
	ctx.SendLogs("DOKSClusterController: TerminateCluster.", models.LOGGING_LEVEL_INFO, models.Backend_Logging)
